...
# # =============================================================================
# echo ""
# echo "=== 1. 重新跑 MyFlow-Gene2Vec 实验 ==="

# source ~/miniconda3/etc/profile.d/conda.sh 2>/dev/null || true
# conda activate flow

# # Norman Additive
# echo "Starting: MyFlow-Gene2Vec Norman Additive"
# CUDA_VISIBLE_DEVICES=0 nohup python "$MYFLOW_DIR/scripts/train_myflow_norman_scdfm_additive.py" \
#     --output-dir "results/outputs/outputs_myflow_gene2vec_norman_additive_${RUN_ID}" \
#     > "$LOG_DIR/myflow_gene2vec_norman_additive.log" 2>&1 &
# echo "  PID: $!"

# # Norman Holdout
# echo "Starting: MyFlow-Gene2Vec Norman Holdout"
# CUDA_VISIBLE_DEVICES=1 nohup python "$MYFLOW_DIR/scripts/train_myflow_norman_scdfm_holdout.py" \
#     --output-dir "results/outputs/outputs_myflow_gene2vec_norman_holdout_${RUN_ID}" \
#     > "$LOG_DIR/myflow_gene2vec_norman_holdout.log" 2>&1 &
# echo "  PID: $!"

# # LOCO
# echo "Starting: MyFlow-Gene2Vec LOCO"
# CUDA_VISIBLE_DEVICES=2 nohup python "$MYFLOW_DIR/scripts/train_myflow_loco_new.py" \
#     --output-dir "results/outputs/outputs_myflow_gene2vec_loco_${RUN_ID}" \
#     > "$LOG_DIR/myflow_gene2vec_loco.log" 2>&1 &
# echo "  PID: $!"

# echo ""
# echo "MyFlow-Gene2Vec experiments launched!"
# echo ""
...
# 创建修复后的评估脚本
cat > "$MYFLOW_DIR/scripts/fix_scdfm_eval.py" << 'EOF'
#!/usr/bin/env python3
"""
修复 scDFM 评估问题：
1. 数据尺度不匹配
...
cat > "$MYFLOW_DIR/scripts/postprocess_des.py" << 'EOF'
#!/usr/bin/env python3
"""
DES 后处理方案：
...
python "$MYFLOW_DIR/scripts/fix_scdfm_eval.py" > "$LOG_DIR/scdfm_eval_fix.log" 2>&1 &
echo "  PID: $!"
