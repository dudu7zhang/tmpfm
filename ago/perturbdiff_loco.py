...
train_holdout_perts_seen = set(
    adata_train_full.obs.loc[
        (adata_train_full.obs["cell_type"] == HOLDOUT) & (~adata_train_full.obs["control"].astype(bool)),
        "target_gene",
    ].astype(str)
)
...
if not ((adata_train_full.obs["cell_type"] == HOLDOUT) & adata_train_full.obs["control"].astype(bool)).any():
...
ctrl_adata = adata_train_full[
    (adata_train_full.obs["control"].astype(bool)) & (adata_train_full.obs["cell_type"] == HOLDOUT)
].copy()
