...
from myflow._logging import logger
from myflow._types import ArrayLike
from myflow.data._data import ConditionData, PredictionData, ReturnData, TrainingData, ValidationData
...
class DataManager:
    ...
    def __init__(
        ...
        self._perturb_covar_keys = list(dict.fromkeys(k for k in perturb_covar_keys if k is not None))
    ...
    def get_prediction_data(
        ...
        covariate_data
            ...
            in :meth:`myflow.model.MyFlow.prepare_data` and stored in
    ...
    def _get_perturbation_covariates_embeddings(
        ...
        perturb_indices: list[np.ndarray] = []
        symbol_to_idx: dict[str, int] = rep_dict.get("combined_perturb_symbol_to_idx", rep_dict.get("perturb_gene_symbol_to_idx", {}))
        ...
        for primary_cov in primary_covars:
            ...
            if primary_group in covariate_reps:
                ...
                rep_map = rep_dict.get(rep_key, {})
                if cov_name not in rep_map:
                    logger.warning(
                        f"Representation for '{cov_name}' not found in `adata.uns['{rep_key}']`. Using null vector.")
                    rep_vals = list(rep_map.values())
                    rep_dim = np.asarray(rep_vals[0]).shape[-1] if len(rep_vals) > 0 else 1
                    prim_arr1 = np.full((1, rep_dim), null_value)
                else:
                    prim_arr1 = np.asarray(rep_map[cov_name])
                if symbol_to_idx and cov_name in symbol_to_idx:
                    perturb_indices.append(np.array([[symbol_to_idx[cov_name]]], dtype=np.int32))
                else:
                    perturb_indices.append(np.array([[-1]], dtype=np.int32))
            else:
            ...
            for linked_covar in linked_perturb_covars[primary_cov].items():
                ...
                if linked_group in covariate_reps:
                    ...
                    if cov_name not in rep_map:
                        ...
                        rep_vals = list(rep_map.values())
                        rep_dim = np.asarray(rep_vals[0]).shape[-1] if len(rep_vals) > 0 else 1
                        linked_arr = np.full((1, rep_dim), null_value)
                    else:
                        linked_arr = np.asarray(rep_map[cov_name])
                else:
        ...
        if perturb_indices:
            idx_arr = np.concatenate(perturb_indices, axis=0)
            ...
            # Emit gene_perturbation_indices if symbol_to_idx is available
            if symbol_to_idx:
    ...
    def _get_sample_covariates_embedding(
        ...
        for sample_cov in sample_covariates:
            ...
            if sample_cov in covariate_reps:
                ...
                rep_map = rep_dict.get(rep_key, {})
                if value not in rep_map:
                    logger.warning(
                        f"Representation for '{value}' not found in `adata.uns['{rep_key}']`. Using null vector.")
                    rep_vals = list(rep_map.values())
                    rep_dim = np.asarray(rep_vals[0]).shape[-1] if len(rep_vals) > 0 else 1
                    cov_arr = np.full((1, rep_dim), null_value)
                else:
                    cov_arr = np.asarray(rep_map[value])
            else:
    ...
    def _get_condition_data(
        ...
        if rep_dict and ("combined_perturb_symbol_to_idx" in rep_dict or "perturb_gene_symbol_to_idx" in rep_dict):
            condition_data["gene_perturbation_indices"] = []
        ...
        perturbation_covariates_keys = list(dict.fromkeys(
            key for key in self.perturb_covar_keys if key not in uniq_sample_keys
        ))

        all_combs_keys = list(dict.fromkeys(uniq_sample_keys + perturbation_covariates_keys))
        if len(self.split_covariates) == 0:
            all_combs_keys = list(dict.fromkeys(perturbation_covariates_keys + uniq_sample_keys))

        df_cols = list(dict.fromkeys(uniq_sample_keys + perturbation_covariates_keys + [self.control_key]))
        df = covariate_data[df_cols].copy()
        ...
        sort_keys = list(dict.fromkeys([*uniq_sample_keys, *perturbation_covariates_keys, self.control_key]))
        ddf = ddf.sort_values(by=sort_keys)
        ddf = ddf.reset_index(drop=True)

        select_keys = list(dict.fromkeys(uniq_sample_keys + perturbation_covariates_keys + [self.control_key]))
        all_combs = ddf[select_keys].drop_duplicates(
            keep="first", subset=select_keys
        )
        ctrl_keys = list(dict.fromkeys(uniq_sample_keys + [self.control_key]))
        control_combs = all_combs[ctrl_keys].drop_duplicates(
            keep="first", subset=ctrl_keys
        )
        with ProgressBar():
