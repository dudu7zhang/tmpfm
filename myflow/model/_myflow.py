...
from myflow import _constants
from myflow._types import ArrayLike, Layers_separate_input_t, Layers_t
from myflow.data._data import ConditionData, TrainingData, ValidationData
from myflow.data._dataloader import OOCTrainSampler, PredictionSampler, TrainSampler, ValidationSampler
from myflow.data._datamanager import DataManager
from myflow.model._utils import _write_predictions
from myflow.networks import _velocity_field
from myflow.plotting import _utils
from myflow.solvers import _genot, _otfm
# from myflow.utils import build_adj_matrix, build_condition_gene_masks  # unused: GRN mask path not active
from myflow.training._callbacks import BaseCallback
...
class MyFlow:
    """MyFlow model for perturbation prediction using Flow Matching and Optimal Transport.

    MyFlow builds upon neural optimal transport estimators extending :cite:`tong:23`,
    ...
    ----------
        ...
        solver
            Solver to use for training. Currently only ``'otfm'`` is supported in this fork.
    """

    def __init__(self, adata: ad.AnnData, solver: Literal["otfm", "genot"] = "otfm"):
        if solver != "otfm":
            raise NotImplementedError("GENOT velocity field support was removed; use solver='otfm'.")
        self._adata = adata
        ...
        self._trainer: MyFlowTrainer | None = None
        ...
        self._vf: _velocity_field.ConditionalVelocityField | None = None
    ...
    def prepare_data(
        ...
        """Prepare the dataloader for training from :attr:`~myflow.model.MyFlow.adata`.

        Parameters
        ----------
        sample_rep
            Key in :attr:`~anndata.AnnData.obsm` of :attr:`myflow.model.MyFlow.adata` where
        ...
        control_key
            ...
            :attr:`myflow.model.MyFlow.adata` that defines the control samples.
        perturbation_covariates
            A dictionary where the keys indicate the name of the covariate group and the values are
            keys in :attr:`~anndata.AnnData.obs` of :attr:`myflow.model.MyFlow.adata`. The
        ...
        - :attr:`myflow.model.MyFlow.data_manager` - the :class:`myflow.data.DataManager` object.
        - :attr:`myflow.model.MyFlow.train_data` - the training data.
    ...
    def prepare_validation_data(
        ...
        name
            ...
            :attr:`myflow.model.MyFlow.validation_data`.
        ...
        predict_kwargs
            ...
            :func:`myflow.solvers._otfm.OTFlowMatching.predict` or
            :func:`myflow.solvers._genot.GENOT.predict` used during validation.

        Returns
        -------
        :obj:`None`, and updates the following fields:

        - :attr:`myflow.model.MyFlow.validation_data` - a dictionary with the validation data.
    ...
    def prepare_model(
        ...
        :attr:`solver`. When :attr:`solver` is an instance of :class:`myflow.solvers._genot.GENOT`,
        ...
        pooling_kwargs
            ...
            - :class:`myflow.networks.TokenAttentionPooling` if ``'pooling'`` is
              ``'attention_token'``.
            - :class:`myflow.networks.SeedAttentionPooling` if ``'pooling'`` is ``'attention_seed'``.
        ...
        layers_before_pool
            ...
            - :class:`tuple` with elements corresponding to dictionaries with keys:
                ...
                - Further keyword arguments for the layer type :class:`myflow.networks.MLPBlock` or
                  :class:`myflow.networks.SelfAttentionBlock`.
        ...
        layers_after_pool
            ...
            - ``'layer_type'`` of type :class:`str` indicating the type of the layer, can be
              ``'mlp'`` or ``'self_attention'``.
            - Further keys depend on the layer type, either for :class:`myflow.networks.MLPBlock` or
              for :class:`myflow.networks.SelfAttentionBlock`.

        condition_embedding_dim
            Dimensions of the condition embedding, i.e. the last layer of the
            :class:`myflow.networks.ConditionEncoder`.
        cond_output_dropout
            Dropout rate for the last layer of the :class:`myflow.networks.ConditionEncoder`.
        condition_encoder_kwargs
            Keyword arguments for the :class:`myflow.networks.ConditionEncoder`.
        ...
        time_max_period
            ...
            :func:`myflow.networks.utils.sinusoidal_time_encoder`.
        time_encoder_dims
            Dimensions of the layers processing the time embedding in
            :attr:`myflow.networks.ConditionalVelocityField.time_encoder`.
        time_encoder_dropout
            Dropout rate for the :attr:`myflow.networks.ConditionalVelocityField.time_encoder`.
        hidden_dims
            Dimensions of the layers processing the input to the velocity field
            via :attr:`myflow.networks.ConditionalVelocityField.x_encoder`.
        hidden_dropout
            Dropout rate for :attr:`myflow.networks.ConditionalVelocityField.x_encoder`.
        conditioning
        ...
        decoder_dims
            ...
            :attr:`myflow.networks.ConditionalVelocityField.decoder`.
        decoder_dropout
            Dropout rate for the output layer
            :attr:`myflow.networks.ConditionalVelocityField.decoder`.
        vf_act_fn
            Activation function of the :class:`myflow.networks.ConditionalVelocityField`.
        vf_kwargs
            ...
            For instance, when ``'solver==genot'``, the following keyword argument can be passed:
                ...
                - ``'genot_source_dims'`` of type :class:`tuple` with the dimensions
                  of the :class:`myflow.networks.MLPBlock` processing the source cell.
        ...
        match_fn
            ...
            :func:`myflow.utils.match_linear`.
        optimizer
            Optimizer used for training.
        solver_kwargs
            Keyword arguments for the solver :class:`myflow.solvers.OTFlowMatching` or
            :class:`myflow.solvers.GENOT`.
        ...
        - :attr:`myflow.model.MyFlow.velocity_field` - an instance of the
          :class:`myflow.networks.ConditionalVelocityField`.
        - :attr:`myflow.model.MyFlow.solver` - an instance of :class:`myflow.solvers.OTFlowMatching`
          or :class:`myflow.solvers.GENOT`.
        - :attr:`myflow.model.MyFlow.trainer` - an instance of the
          :class:`myflow.training.MyFlowTrainer`.
        """
        if self.train_data is None:
        ...
        self.vf = self._vf_class(
            ...
            gene_attn_dim=gene_attn_dim,
            gene_self_attn_layers=gene_self_attn_layers,
        ...
        self._trainer = MyFlowTrainer(solver=self.solver, predict_kwargs=self.validation_data["predict_kwargs"])  # type: ignore[arg-type]
    ...
    def train(
        ...
        callbacks
            ...
            - Callbacks for computations should inherit from
              :class:`~myflow.training.ComputationCallback` see e.g. :class:`myflow.training.Metrics`.
            - Callbacks for logging should inherit from :class:`~myflow.training.LoggingCallback` see
              e.g. :class:`~myflow.training.WandbLogger`.
        monitor_metrics
            Metrics to monitor.
        out_of_core_dataloading
            If :obj:`True`, use out-of-core dataloading. Uses the :class:`myflow.data._dataloader.OOCTrainSampler`
        ...
        - :attr:`myflow.model.MyFlow.dataloader` - the training dataloader.
        - :attr:`myflow.model.MyFlow.solver` - the trained solver.
        """
        if self.train_data is None:
    ...
    def predict(
        ...
        **kwargs: Any,
        ...
        covariate_data
            ...
            :attr:`myflow.model.MyFlow.adata`, and as registered in
            :attr:`myflow.model.MyFlow.data_manager`.
        sample_rep
        ...
        key_added_prefix
            ...
            :class:`dict`.
        rng
            Random number generator. If :obj:`None` and :attr:`myflow.model.MyFlow.conditino_mode`
        ...
        kwargs
            ...
            :meth:`myflow.solvers.OTFlowMatching.predict` or :meth:`myflow.solvers.GENOT.predict`.
    ...
    def get_condition_embedding(
        ...
        generated by the :class:`~myflow.networks.ConditionEncoder`.
        ...
        covariate_data
            ...
            - a :class:`~pandas.DataFrame` defining the conditions with the same columns as the
              :class:`~anndata.AnnData` used for the initialisation of :class:`~myflow.model.MyFlow`.
            - an instance of :class:`~myflow.data.ConditionData`.
    ...
    def save(
        ...
        Pickles the :class:`~myflow.model.MyFlow` object.
    ...
    def load(
        ...
        """
        Load a :class:`~myflow.model.MyFlow` model from a saved instance.
    ...
    @property
    def trainer(self) -> MyFlowTrainer | None:
    ...
    def data_manager(self) -> DataManager:
        """The data manager, initialised with :attr:`myflow.model.MyFlow.adata`."""
        return self._dm

    @property
    def velocity_field(
        self,
    ) -> _velocity_field.ConditionalVelocityField | None:
