...
from myflow.data._dataloader import OOCTrainSampler, TrainSampler, ValidationSampler
from myflow.solvers import _genot, _otfm
from myflow.training._callbacks import BaseCallback, CallbackRunner
...
class MyFlowTrainer:
    ...
    ----------
        dataloader
            Data sampler.
        solver
            :class:`~myflow.solvers._otfm.OTFlowMatching` or
            :class:`~myflow.solvers._genot.GENOT` solver with a conditional velocity field.
        predict_kwargs
    ...
    def train(
        ...
        for it in pbar:
            ...
            if (valid_freq > 0) and ((it - 1) % valid_freq == 0) and (it > 1):
            ...
            # Always update progress by training-set loss (smoothed), independent of valid_freq.
            loss_window = min(loss_smoothing_window, len(self.training_logs["loss"]))
            mean_loss = np.mean(self.training_logs["loss"][-loss_window:])
            postfix_dict = {"loss": round(mean_loss, 3)}
            for metric in monitor_metrics:
                if metric in self.training_logs and len(self.training_logs[metric]) > 0:
                    postfix_dict[metric] = round(self.training_logs[metric][-1], 3)
