...
from myflow.metrics._metrics import (
...
class BaseCallback(abc.ABC):
    """Base class for callbacks in the :class:`~myflow.training.MyFlowTrainer`"""
...
class LoggingCallback(BaseCallback, abc.ABC):
    """Base class for logging callbacks in the :class:`~myflow.training.MyFlowTrainer`"""
...
class ComputationCallback(BaseCallback, abc.ABC):
    """Base class for computation callbacks in the :class:`~myflow.training.MyFlowTrainer`"""
    ...
    def on_log_iteration(
        ...
        solver
            :class:`~myflow.solvers.OTFlowMatching` solver or :class:`~myflow.solvers.GENOT`
...
class VAEDecodedMetrics(Metrics):
    ...
    vae
        ...
        of :class:`myflow.external.CFJaxSCVI`.
    adata
...
class CallbackRunner:
    """Runs a set of computational and logging callbacks in the :class:`~myflow.training.MyFlowTrainer`

    Parameters
    ----------
    callbacks
        List of callbacks to run. Callbacks should be of type
        :class:`~myflow.training.ComputationCallback` or
        :class:`~myflow.training.LoggingCallback`
