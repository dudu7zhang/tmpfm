...
import scipy
class sampler:
    def __init__(self,model_path,gene_size,output_dim,use_drug_structure,class_cond=False,num_classes=None):
        args = self.parse_args(model_path,gene_size,output_dim,use_drug_structure,class_cond,num_classes)
    ...
    def parse_args(self,model_path,gene_size,output_dim,use_drug_structure,class_cond=False,num_classes=None):
    ...
    def pred(self, z_sem, gene_size, perturbation_label=None):
        """
        Generate predictions.

        Args:
            z_sem: latent embedding tensor
            gene_size: number of output genes
            perturbation_label: optional integer tensor of shape [batch] for perturbation conditioning
        """
    ...
    def encode_with_perturbation(self, control_x, perturbation_label):
        """
        Encode control cells with perturbation conditioning.

        Args:
            control_x: control cell expression tensor [batch, genes]
            perturbation_label: integer tensor of shape [batch] or [batch, 1]

        Returns:
            z_sem: latent embedding conditioned on perturbation
        """
        if perturbation_label.dim() == 1:
            perturbation_label = perturbation_label.unsqueeze(1)
        return self.model.encoder(control_x, label=perturbation_label)
