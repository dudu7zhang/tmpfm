#!/usr/bin/env python3
"""
GEARS on Norman 2019 dataset with holdout split.
...
_BASE_OUTPUT_DIR = Path(__file__).resolve().parent.parent.parent / "results" / "outputs" / "outputs_gears_norman_holdout"
...
print(f"[GEARS-Norman-Holdout] Start at {timestamp}")
...
# Train = control + train pert; Test = test pert only (no control for zero-shot)
...
pert_data.new_data_process(dataset_name="norman_gears_holdout", adata=adata_combined, skip_calc_de=False)
pert_data.load(data_path=os.path.join(data_path, "norman_gears_holdout"))
...
for cond in sorted(test_conditions):
    ...
    if genes:
        # Skip perturbations with genes not in the GO graph (not in pert_list)
        if any(g not in gears_model.pert_list for g in genes):
            skipped_perts.append(genes)
        else:
            test_pert_list.append(genes)

if skipped_perts:
    print(f"Skipping {len(skipped_perts)} perturbations with genes not in GEARS GO graph: {skipped_perts}")
print(f"Predicting {len(test_pert_list)} perturbations...")

if not test_pert_list:
    print("No test perturbations are predictable. Skipping prediction.")
...
# ===================== Evaluation =====================
print("\n" + "=" * 50)
evaluate_predictions(ctrl_adata, adata_test_g, adata_pred, str(OUTPUT_DIR / f"gears_norman_holdout_{timestamp}"))
print("=" * 50)
print("Done.")
