#!/usr/bin/env python3
"""
PerturbDiff on Norman 2019 dataset with holdout split.
...
_BASE_OUTPUT_DIR = Path(__file__).resolve().parent.parent.parent / "results" / "outputs" / "outputs_perturbdiff_norman_holdout"
...
print(f"[PerturbDiff-Norman-Holdout] Start at {timestamp}")
...
train_conditions, test_conditions, split_info = build_scdfm_norman_split(
    ...
    test_fraction=0.3, seed_base=SPLIT_SEED_BASE,
...
# Holdout: test = test pert only (zero-shot)
...
# ===================== Evaluation =====================
print("\n" + "=" * 50)
evaluate_predictions(ctrl_adata, adata_test, adata_pred, str(OUTPUT_DIR / f"perturbdiff_norman_holdout_{timestamp}"))
print("=" * 50)
print("Done.")
