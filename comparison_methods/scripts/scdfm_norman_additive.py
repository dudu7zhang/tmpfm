#!/usr/bin/env python3
"""
scDFM on Norman 2019 dataset.
Uses scDFM's own Data class with additive split (fold=0).
...
if not os.path.exists(ADATA_PATH):
    for _path in (
        ...
        if os.path.exists(_path):
            ADATA_PATH = _path
            break
_BASE_OUTPUT_DIR = Path(__file__).resolve().parent.parent.parent / "results" / "outputs" / "outputs_scdfm_norman_additive"
...
print(f"[scDFM-Norman-Additive] Start at {timestamp}")
...
# Use a split-local cache directory for scDFM-generated masks/files.
...
data_manager.process_data(
    ...
    infer_top_gene=data_manager.adata.n_vars,  # Keep all genes for evaluation; INFER_TOP_GENE is for training chunking only
...
vf.eval()
gene_ids_all = torch.tensor(vocab.encode(list(data_manager.adata_test.var_names)), dtype=torch.long, device=device)
n_genes_total = gene_ids_all.numel()
# Chunk indices for memory-safe prediction (attention is O(n^2) in gene count)
gene_chunks = [torch.arange(i, min(i + INFER_TOP_GENE, n_genes_total), device=device)
...
for pert_name in test_sampler._perturbation_covariates:
    ...
    for i in range(0, src.shape[0], BATCH_SIZE):
        ...
        # Predict in chunks to avoid OOM (attention is O(genes^2))
        chunk_preds = []
        for chunk_idx in gene_chunks:
            chunk_src = batch_src[:, chunk_idx]
            chunk_gene_ids = gene_ids_all[chunk_idx]
            chunk_pred = generate_sample(wrapped_vf_fn, chunk_src, batch_pert, vf,
...
# ===================== Evaluation =====================
# adata_test has all 2000 genes (process_data with infer_top_gene=2000).
# Predictions are chunked over all 2000 genes, no ctrl padding.
adata_real = data_manager.adata_test.copy()
ctrl_eval = data_manager.adata_train[data_manager.adata_train.obs["is_control"]].copy()

print("\n" + "=" * 50)
evaluate_predictions(ctrl_eval, adata_real, adata_pred, str(OUTPUT_DIR / f"scdfm_norman_additive_{timestamp}"))
print("=" * 50)
print("Done.")
