...
_BASE_OUTPUT_DIR = Path(__file__).resolve().parent.parent.parent / "results" / "outputs" / "outputs_perturbdiff_norman_additive"
...
print(f"[PerturbDiff-Norman-Additive] Start at {timestamp}")
...
def ddim_sample(model, n_samples, input_dim, cont, cov, n_steps=100, device="cuda"):
    ...
    timesteps = torch.arange(0, 1000, step_size, device=device).flip(0)  # [999, 996, ..., 0]
...
# ===================== Evaluation =====================
print("\n" + "=" * 50)
evaluate_predictions(ctrl_adata, adata_test, adata_pred, str(OUTPUT_DIR / f"perturbdiff_norman_additive_{timestamp}"))
print("=" * 50)
print("Done.")
