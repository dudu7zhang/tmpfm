#!/bin/bash
# Run GEARS on Norman 2019 with additive split
...
echo "Starting GEARS Norman Additive at $(date)"
echo "Run ID: $RUN_ID"
echo "Log file: $LOG_DIR/gears_norman_additive.log"

nohup python "$SCRIPTS_DIR/gears_norman_additive.py" \
    > "$LOG_DIR/gears_norman_additive.log" 2>&1 &

echo "PID: $!"
echo "Monitor with: tail -f $LOG_DIR/gears_norman_additive.log"
