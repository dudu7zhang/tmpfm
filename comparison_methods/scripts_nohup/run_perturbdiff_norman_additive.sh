#!/bin/bash
# Run PerturbDiff on Norman 2019 with additive split
...
echo "Starting PerturbDiff Norman Additive at $(date)"
echo "Run ID: $RUN_ID"
echo "Log file: $LOG_DIR/perturbdiff_norman_additive.log"

nohup python "$SCRIPTS_DIR/perturbdiff_norman_additive.py" \
    > "$LOG_DIR/perturbdiff_norman_additive.log" 2>&1 &

echo "PID: $!"
echo "Monitor with: tail -f $LOG_DIR/perturbdiff_norman_additive.log"
