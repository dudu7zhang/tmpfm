#!/bin/bash
# Run PerturbDiff on Norman 2019 with holdout split
...
echo "Starting PerturbDiff Norman Holdout at $(date)"
echo "Run ID: $RUN_ID"
echo "Log file: $LOG_DIR/perturbdiff_norman_holdout.log"

nohup python "$SCRIPTS_DIR/perturbdiff_norman_holdout.py" \
    > "$LOG_DIR/perturbdiff_norman_holdout.log" 2>&1 &

echo "PID: $!"
echo "Monitor with: tail -f $LOG_DIR/perturbdiff_norman_holdout.log"
