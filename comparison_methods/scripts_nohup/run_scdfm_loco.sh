#!/bin/bash
# Run scDFM on Replogle with LOCO split
...
echo "Starting scDFM LOCO at $(date)"
echo "Run ID: $RUN_ID"
echo "Log file: $LOG_DIR/scdfm_loco.log"

nohup python "$SCRIPTS_DIR/scdfm_loco.py" \
    > "$LOG_DIR/scdfm_loco.log" 2>&1 &

echo "PID: $!"
echo "Monitor with: tail -f $LOG_DIR/scdfm_loco.log"
