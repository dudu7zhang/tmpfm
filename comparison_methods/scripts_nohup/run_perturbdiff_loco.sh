#!/bin/bash
# Run PerturbDiff on Replogle with LOCO split
...
echo "Starting PerturbDiff LOCO at $(date)"
echo "Run ID: $RUN_ID"
echo "Log file: $LOG_DIR/perturbdiff_loco.log"

nohup python "$SCRIPTS_DIR/perturbdiff_loco.py" \
    > "$LOG_DIR/perturbdiff_loco.log" 2>&1 &

echo "PID: $!"
echo "Monitor with: tail -f $LOG_DIR/perturbdiff_loco.log"
