#!/bin/bash
# Run GEARS on Norman 2019 with holdout split
...
echo "Starting GEARS Norman Holdout at $(date)"
echo "Run ID: $RUN_ID"
echo "Log file: $LOG_DIR/gears_norman_holdout.log"

nohup python "$SCRIPTS_DIR/gears_norman_holdout.py" \
    > "$LOG_DIR/gears_norman_holdout.log" 2>&1 &

echo "PID: $!"
echo "Monitor with: tail -f $LOG_DIR/gears_norman_holdout.log"
