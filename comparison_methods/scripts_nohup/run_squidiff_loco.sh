#!/bin/bash
# Run Squidiff on Replogle with LOCO split
...
echo "Starting Squidiff LOCO at $(date)"
echo "Run ID: $RUN_ID"
echo "Log file: $LOG_DIR/squidiff_loco.log"

nohup python "$SCRIPTS_DIR/squidiff_loco.py" \
    > "$LOG_DIR/squidiff_loco.log" 2>&1 &

echo "PID: $!"
echo "Monitor with: tail -f $LOG_DIR/squidiff_loco.log"
