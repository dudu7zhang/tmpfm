...
class OTPlanSampler:
    ...
    def sample_plan_fix_x0(self, x0, x1, replace=True):
        """
        Sample matched samples from x1 according to the OT plan, keeping the order of x0 unchanged.

        Args:
            x0 : Tensor, shape (bs, *dim) - kept unchanged
            x1 : Tensor, shape (bs, *dim) - to be sampled
            replace : bool - whether to sample with replacement

        Returns:
            x0 : original x0, unchanged
            x1_matched : matched samples sampled from x1
        """
        pi = self.get_map(x0, x1)
        matched_indices = []
