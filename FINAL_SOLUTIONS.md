...
3. CellFlow-Gene2Vec 的预测数据方差很低但不是 0，导致 t-test 无法检测到显著差异

### 解决方案：增强 DES 计算
创建了 `fix_des_for_cellflow.py`，包含：
...
- `fix_des_for_cellflow.py` - 增强 DES 计算方案
...
2. **CellFlow-Gene2Vec 的 DES 低** 是因为预测数据方差太低
