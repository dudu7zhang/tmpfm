#!/usr/bin/env python3
"""
Train CellFlow on the Norman et al. 2019 combinatorial perturbation dataset.

Key adaptations from the replogle/zebrafish scripts:
- Combinatorial perturbation embeddings: splits guide_merged by '+', averages gene2vec per gene
- Perturbation condition split (not LOCO) since Norman is single cell type (K562)
- guide_identity as target_gene, guide_merged as condition, 'ctrl' as control marker
"""
...
def build_scdfm_norman_split(
    ...
    holdout: hold out single genes; test contains those single perturbations
    ...
    if not test_conditions:
        raise ValueError(f"No test conditions generated for split_method={split_method}")
    if not train_conditions:
        raise ValueError(f"No train conditions generated for split_method={split_method}")
...
def parse_args():
    ...
    p.add_argument(
        "--split-method",
        choices=["additive", "holdout", "unseen"],
        default="additive",
        help="Norman split protocol. additive matches scDFM's additive setting; holdout/unseen withholds genes.",
    )
    ...
    p.add_argument("--holdout-genes-count", type=int, default=12)
...
def main():
    ...
    print(f"  Split method: {args.split_method}, fold={args.fold}, split_seed={args.split_seed_base + args.fold}")
    ...
    try:
        ...
        print("\nCalculating global DES & DE-Spearman...")
